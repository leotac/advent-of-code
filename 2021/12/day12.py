...
def explore(cur, visited, exceptions, path):
    ...
    if exceptions > 0:
        return sum(
                explore(n, visited | {n}, exceptions, path+[n]) for n in G[cur] 
                if (big(n) or (n not in visited)) and n != "start" ) \
                + sum(
                    explore(n, visited | {n}, exceptions-1, path+[n]) for n in G[cur]
                    if not big(n) and (n in visited) and n != "start")
    else: # no exceptions remaining
        return sum(explore(n, visited | {n}, exceptions, path+[n]) for n in G[cur] 
                if (big(n) or (n not in visited)) and n != "start" )

def main(filename, exceptions):
    visited = set()
    return explore("start", visited, exceptions, ["start"])
